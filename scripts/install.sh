...
# Optional vars
APP_NAME="${APP_NAME:="arbiter"}"
GITHUB_REPO="${GITHUB_REPO:="initializ/arbiter"}"
...
OPTIONS:
   -h      Show help.
   -d      Directory where arbiter will be installed. Default is /usr/local/bin
...
download_version() {
  ...
  curl -L -o- "https://github.com/${GITHUB_REPO}/releases/download/${version}/${APP_NAME}-${version}-${TARGET_ARCH}-${TARGET_OS}.tar.gz" \
